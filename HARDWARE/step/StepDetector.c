...
bool detectorPeak(float newValue, float oldValue) 
{
        ...
        if (!isDirectionUp && lastStatus
                && (continueUpFormerCount >= 2 && (oldValue >= 10.78&& oldValue < 19.6))) {		// 10.78 19.6
            peakOfWave = oldValue;
            return true;
...
//		}
		if(flag)
		{
			//steplength+=0.37f+((timeOfThisPeak-timeOfLastPeak)*10)/10000+0.4f*(peakOfWave-valleyOfWave)/5;
			steplength+=0.37f-0.000155f*(timeOfThisPeak-timeOfLastPeak)*10+0.16385f*(float) sqrt(peakOfWave-valleyOfWave);
			flag=0;
			
		}
//		OLED_P6x8Num(0,0,mCount,8);
//		OLED_P6x8Num(0,2,gravityNew,8);
//		OLED_P6x8Num(0,4,steplength,8);
//		OLED_P6x8Num(15,4,(steplength-(int)steplength)*100,8);
		//averageValue(tempValue,4);
		StepDetector_tick++;
//		delay_ms(10);
//	}
}
